(() => {
  ...
  class Deck {
    constructor(storageKey, ids) {
      ...
      this.order = saved.order && saved.order.length === ids.length ? saved.order : this._shuffle([...ids]);
  ...
  async function loadData() {
    const [dsa, sysdesign, ml] = await Promise.all([
      fetch('data/dsa.json').then(r => r.json()),
      fetch('data/system_design.json').then(r => r.json()),
      fetch('data/ml.json').then(r => r.json())
    ]);
    state.dsa = dsa;
    state.sysdesign = sysdesign;
    state.ml = ml;

    state.decks.dsa = new Deck('drillset_dsa_state', dsa.problems.map(p => p.id));
    state.decks.sysdesign = new Deck('drillset_sysdesign_state', sysdesign.map(q => q.id));
    state.decks.ml = new Deck('drillset_ml_state', ml.map(c => c.id));
  ...
  loadData().catch(err => {
    document.getElementById('view-home').innerHTML =
      `<p style="padding:40px 4px;color:#F2726F;font-family:monospace;">
        Couldn't load flashcard data (${err.message}). If you're running this locally,
        serve the folder with a local web server rather than opening index.html directly —
        e.g. <code>python3 -m http.server</code> — since JSON fetches need http(s).
      </p>`;
  });
})();
